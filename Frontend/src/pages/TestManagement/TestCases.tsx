...
function SuiteGroup({
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm overflow-hidden mb-2">
      {/* Suite header */}
      <div
        className="flex items-center justify-between px-5 py-3.5 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/60 transition-colors duration-150"
        onClick={() => setOpen((v) => !v)}
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <span className="text-xs text-gray-400 dark:text-gray-500 flex-shrink-0">
            {cases.length} {cases.length === 1 ? "test case" : "test cases"}
          </span>
          {suite.project_name && (
...
export default function TestCases() {
  ...
  const suiteGroups =
    allSuites
      ?.map((suite) => ({
        suite,
        cases: (testCases || []).filter((tc) => tc.suite_id === suite.id),
      }))
