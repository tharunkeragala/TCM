...
export default function TasksReport() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      total: all.length,
      open: all.filter((t) => t.status.toLowerCase() === "open").length,
      inProgress: all.filter((t) => t.status.toLowerCase() === "in progress")
        .length,
      done: all.filter((t) => t.status.toLowerCase() === "done").length,
