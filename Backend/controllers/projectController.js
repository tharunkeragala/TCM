...
exports.getProjectSuiteCount = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to fetch suite count", error: err.message });
  }
};
...
exports.toggleProject = async (req, res) => {
  try {
    ...
    await pool
      .request()
      .input("description", sql.VarChar, `Project ID ${id} status changed to ${is_active ? "Active" : "Inactive"}`)
      .input("user_id", sql.Int, userId)
      .query(`
        INSERT INTO audit_logs (action, module, description, user_id)
