...
exports.processReminders = async () => {
  try {
      ...
      result.recordset.map(async (reminder) => {
        ...
        await pool
          ...
          .input(
            "message",
            sql.NVarChar,
            `Reminder: Task "${reminder.title}" is due on ${reminder.due_date}`,
          ).query(`
        ...
        if (reminder.is_recurring) {
          ...
          if (reminder.remind_unit === "hours") {
            nextRemindAt.setHours(
              nextRemindAt.getHours() + reminder.remind_before,
            );
          }

          if (reminder.remind_unit === "days") {
            nextRemindAt.setDate(
              nextRemindAt.getDate() + reminder.remind_before,
            );
          }

          if (reminder.remind_unit === "months") {
            nextRemindAt.setMonth(
              nextRemindAt.getMonth() + reminder.remind_before,
            );
          }
          ...
          await pool
            ...
            .input("remind_at", sql.DateTime, nextRemindAt).query(`
          ...
          await pool.request().input("id", sql.Int, reminder.id).query(`
              UPDATE test_case_manager.dbo.task_reminders
              SET is_sent = 1
              WHERE id = @id
            `);
    ...
    const overdue = await pool.request().query(`
      ...
      JOIN test_case_manager.dbo.task_assignments ta
        ON ta.task_id = t.id
    ...
    console.log(
      `[Reminder Processor] Processed ${result.recordset.length} reminders, ${overdue.recordset.length} overdue alerts`,
    );
  } catch (err) {
