...
function parseTestScript(script = '') {
  ...
  for (const line of lines) {
    ...
    if ((m = t.match(/await page\.goto\(['"`](.+?)['"`]/))) {
      ...
      steps.push({ action: 'click', selector: parseSelectorArgument(m[1]), raw: t });
    } else if ((m = t.match(/await page\.fill\((.+),\s*['"`](.*)['"`]\)/))) {
      steps.push({ action: 'fill', selector: parseSelectorArgument(m[1]), value: m[2], raw: t });
    } else if ((m = t.match(/await page\.type\((.+),\s*['"`](.*)['"`]\)/))) {
      steps.push({ action: 'type', selector: parseSelectorArgument(m[1]), value: m[2], raw: t });
    } else if ((m = t.match(/await page\.selectOption\((.+),\s*['"`](.*)['"`]\)/))) {
      steps.push({ action: 'select', selector: parseSelectorArgument(m[1]), value: m[2], raw: t });
    } else if ((m = t.match(/await page\.waitForSelector\(['"`](.+?)['"`]\)/))) {
    ...
    } else if ((m = t.match(/await expect\(page\)\.toHaveTitle\(['"`](.+?)['"`]\)/))) {
...
async function runTestCase(testCaseId, userId = null) {
  ...
  const tcResult = await pool.request()
  ...
  if (!testCase.playwright_script || !String(testCase.playwright_script).trim()) {
  ...
  args: [
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      const stepResult = await pool.request()
        ...
        .input('selector', sql.NVarChar(sql.MAX), step.selector == null ? null : JSON.stringify(step.selector))
      ...
      broadcast({ type: 'step_started', runId, stepId, stepNum: i + 1, step, total: steps.length });
      ...
      try {
        switch (step.action) {
          ...
          case 'fill':
            ...
            break;
          case 'type':
            await resolveLocator(page, step.selector).pressSequentially(step.value || '');
            break;
          case 'select':
            await resolveLocator(page, step.selector).selectOption(step.value || '');
            break;
          case 'waitForSelector':
            await resolveLocator(page, step.selector).waitFor({ state: 'visible' });
            break;
          case 'wait':
          ...
          case 'assertTitle': {
            ...
            if (title !== step.value) throw new Error(`Expected title "${step.value}" but got "${title}"`);
            break;
          }
          case 'screenshot':
            break;
        ...
        await pool.request()
          ...
          .input('screenshot_path', sql.NVarChar, publicScreenshotPath)
          .query(`
    ...
    broadcast({ type: 'run_completed', runId, status: 'failed', error: err.message, duration });
  } finally {
