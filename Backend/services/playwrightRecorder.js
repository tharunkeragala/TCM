...
function generateScript(actions = []) {
  const deduped = actions.reduce((acc, action) => {
    if (
      ...
      JSON.stringify(acc[acc.length - 1].selector) === JSON.stringify(action.selector)
    ) {
...
async function startRecording(url) {
  ...
  await page.exposeFunction('__recordAction', (action) => {
    ...
    if (
      ...
      JSON.stringify(actions[actions.length - 1].selector) === JSON.stringify(action.selector)
    ) {
  ...
  await page.addInitScript(() => {
    function getXPath(el) {
      ...
      while (node && node.nodeType === Node.ELEMENT_NODE) {
        ...
        let siblings = [];
        if (parent) {
          siblings = Array.from(parent.children).filter((c) => c.tagName === node.tagName);
        }
    ...
    document.addEventListener('click', (e) => {
      window.__recordAction({ action: 'click', selector: getSelector(e.target) });
    }, true);
    ...
    document.addEventListener('change', (e) => {
      ...
      if (tag === 'INPUT' || tag === 'TEXTAREA') {
        window.__recordAction({ action: 'fill', selector: sel, value: t.value });
      }
      if (tag === 'SELECT') {
        window.__recordAction({ action: 'select', selector: sel, value: t.value });
      }
    }, true);
    ...
    document.addEventListener('input', (e) => {
      ...
      const tag = t.tagName;
      if (tag === 'INPUT' || tag === 'TEXTAREA') {
        window.__recordAction({ action: 'fill', selector: getSelector(t), value: t.value });
      }
    }, true);
  });
